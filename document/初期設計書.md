...
| ----- | ------------------------------ | ---------------- | --------------------------------------------------------------- | ------------ | ------------------------------------------------------------ |
| CH‑T  | **Teams Webhook**              | Webhook Endpoint | Teams メッセージを標準化JSON形式でAPI Gatewayへ送信             | Lambda Auto  | API Key認証 + SIGv4署名検証                                  |
| CH‑L  | **LINE Messaging API**         | Webhook Endpoint | LINE ボット受付。標準化JSON形式でAPI Gatewayへ送信              | Lambda Auto  | X-LINE-Signature検証 + WAF                                   |
| CH‑S  | **Slack Events API**           | Webhook Endpoint | Slackメッセージを標準化JSON形式でAPI Gatewayへ送信              | Lambda Auto  | Slack署名検証 + 3秒以内応答確認                              |
| CH‑D  | **Discord Webhook**            | Webhook Endpoint | Discordメッセージを標準化JSON形式でAPI Gatewayへ送信            | Lambda Auto  | Discord署名検証                                              |
| CH‑TR | **Webhook Transformer Lambda** | Lambda@Edge      | 各プラットフォーム固有形式→統一JSONスキーマへ変換               | Lambda Auto  | IAMロール最小権限                                            |
...
| ENTR   | **AWS Lambda** (`entry-fn`)     | Python 3.12 / 128 MB / 30 s          | `EVENT_BUS=chatops-bus` | 統一JSONスキーマ処理：<br>`{ platform, channelId, message, userId, etc }` |
| EB     | **EventBridge (custom bus)**    | 名前: `chatops-bus`                  | —                       | ルール: `source in ['teams','line','slack','discord','github']` |
